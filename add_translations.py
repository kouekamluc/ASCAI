#!/usr/bin/env python
"""
Add French and Italian translations to .po files.
This script updates the msgstr entries with proper translations.
"""
import re
...
def update_po_file_with_translations(po_file_path, translations):
    ...
    def replace_translation(match):
        msgid = match.group(1)
        current_msgstr = match.group(2)
        
        # If translation exists and current msgstr is empty or same as msgid, replace it
    ...
    # Also handle multiline msgid/msgstr (simplified)
    # This is a basic implementation - for production, use a proper .po parser
...
def main():
    """Update French and Italian .po files with translations."""
    print("Adding translations to .po files...")
    
    # Update French translations
    fr_po = BASE_DIR / "locale" / "fr" / "LC_MESSAGES" / "django.po"
    if fr_po.exists():
        update_po_file_with_translations(fr_po, FRENCH_TRANSLATIONS)
    else:
    ...
    if it_po.exists():
        update_po_file_with_translations(it_po, ITALIAN_TRANSLATIONS)
    else:
